...
<form className="flex flex-col gap-8 selection:bg-green selection:text-black" onSubmit={handleSubmit}>
  ...
  <div className="flex flex-col gap-2">
    <button className="focus-visible:outline-white rounded-md text-white border-green w-full py-3 cursor-pointer disabled:cursor-not-allowed disabled:opacity-20" aria-live="polite" disabled={isLoading || !username}
    >
      <span>{isLogin ? 'Login' : 'Register'}</span>
    </button>
  </div>

  <button className="flex items-center justify-center cursor-pointer" type="button"
  onClick={() => {
    setIsLogin(!isLogin);
    setError('');
  }}
  >
    <span className="text-[#8E8E8E]" style={{ WebkitTextFillColor: 'currentcolor' }}>
